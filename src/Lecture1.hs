...
lowerAndGreater n xs = show n ++ " is greater than " ++ show less ++ " elements and lower than " ++ show greater ++ " elements"
  where
    less = length $ filter (<n) xs
    greater = length $ filter (>n) xs
